...
### 🌐 Работа за прокси
Прокси выключен по умолчанию, поэтому стандартные команды `mvn …` и `./mvnw …` работают без дополнительных ключей. Чтобы включить корпоративный прокси `proxy:8080`, запускайте Maven с дополнительными опциями `-s .mvn/settings.xml -Dvisitmanager.proxyEnabled=true` — файл `.mvn/settings.xml` активирует HTTP(S)-прокси только при явной передаче этих ключей.【F:.mvn/settings.xml†L1-L22】
...
(Micronaut Test Resources) или `mvn -Pit-external verify` (подключение к реальным сервисам). В средах с корпоративным прокси добавляйте `-Dvisitmanager.proxyEnabled=true` ко всем командам Maven.
