...
5. **Печать талона** – параметр `printTicket=true` при создании визита отправляет талон на `PRINTER_ID` («Intro17») → `200 OK`.

7. **Отмена чужого визита** – `DELETE` с чужим `VISIT_ID` → `403 Forbidden`.
...
3. **Недоступная услуга** – регистрация с некорректным `SERVICE_ID` → `404/409`.
...
5. **Печать талона** – параметр `printTicket=true` при регистрации отправляет талон на `PRINTER_ID` («Intro17») → `200 OK`.
