...
class ManagementControllerE2EIT {
  ...
  @DisplayName("Возвращает отделение по идентификатору")
  @Test
  void returnsBranchById() {
    ...
    Branch fetched =
        client
            .toBlocking()
            .retrieve(HttpRequest.GET("/managementinformation/branches/b1"), Branch.class);
    assertEquals("Branch", fetched.getName());
  }

  @DisplayName("Возвращает краткий список отделений")
  @Test
  void returnsTinyBranches() {
    ...
    TinyClass[] tiny =
        client
            .toBlocking()
            .retrieve(HttpRequest.GET("/managementinformation/branches/tiny"), TinyClass[].class);
