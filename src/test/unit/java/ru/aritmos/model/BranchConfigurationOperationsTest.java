...
class BranchConfigurationOperationsTest {
  ...
  void updateVisitWithCustomActionPlacesEntitiesAndSendsEvents() {
    ...
    assertEquals(
        List.of(queueVisitHead.getId(), queueVisitTail.getId(), visit.getId()),
  ...
  void updateVisitWithCustomActionThrowsWhenPointBusy() {
    ...
    ServicePoint servicePoint =
        new ServicePoint("sp-main", "Основная точка") {
          @Override
    ...
    Visit visit = Visit.builder().id("visit-new").servicePointId(servicePoint.getId()).build();
  ...
  void addUpdateServicePointRestoresStateAndPublishesEvents() {
    ...
    verify(eventService)
        .sendChangedEvent(
            ...
            eq(updated),
            anyMap(),
            eq("Update service point"));
    verify(eventService)
        .sendChangedEvent(
            eq("config"), eq(false), isNull(), eq(newcomer), anyMap(), eq("Add service point"));
  }
  ...
  void addUpdateQueuesRestoresVisitsAndNotifiesConsumers() {
    ...
    assertEquals(
        existingQueue.getVisits().stream().map(Visit::getId).toList(),
    ...
    verify(eventService)
        .sendChangedEvent(
            eq("config"), eq(false), isNull(), eq(newQueue), anyMap(), eq("Add queue"));
    verify(eventService)
        .sendChangedEvent(
            eq("config"), eq(false), isNull(), eq(newQueue), anyMap(), eq("Add service"));
  }
}
