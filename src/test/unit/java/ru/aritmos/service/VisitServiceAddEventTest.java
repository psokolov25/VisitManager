...
class VisitServiceAddEventTest {
  ...
  void addsEventWhenSequenceIsValid() {
    Visit visit = Visit.builder().visitEvents(new ArrayList<>()).events(new ArrayList<>()).build();
  ...
  void firstEventMustBeCreated() {
    ...
    assertThrows(
        HttpStatusException.class, () -> service.addEvent(visit, VisitEvent.CALLED, eventService));
  ...
  void throwsWhenEventCannotFollowPrevious() {
    ...
    service.addEvent(visit, VisitEvent.PLACED_IN_QUEUE, eventService);
    assertThrows(
        HttpStatusException.class, () -> service.addEvent(visit, VisitEvent.CREATED, eventService));
    verify(eventService).send(eq("*"), eq(false), any());
  }
}
