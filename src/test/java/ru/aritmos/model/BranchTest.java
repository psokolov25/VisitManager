...
class BranchTest {
    ...
    @Test
    void deleteServicePointsRemovesPointAndPublishesEvent() {
        // Подготавливаем отделение с одной точкой обслуживания
        Branch branch = new Branch("b1", "Branch");
        ServicePoint sp = new ServicePoint("sp1", "СП1");
        branch.getServicePoints().put("sp1", sp);
        EventService eventService = mock(EventService.class);

        // Удаляем точку обслуживания
        branch.deleteServicePoints(List.of("sp1"), eventService);

        // Точка обслуживания удалена из отделения
        assertTrue(branch.getServicePoints().isEmpty());

        // Отправлено событие удаления точки обслуживания
        verify(eventService).sendChangedEvent(eq("config"), eq(false), isNull(), eq(sp), anyMap(), eq("Delete service point"));
    }

    @Test
    void deleteQueuesRemovesQueueAndPublishesEvent() {
        // Подготавливаем отделение с очередью
        Branch branch = new Branch("b1", "Branch");
        ...
        // Удаляем очередь
        branch.deleteQueues(List.of("q1"), eventService);

        // Очередь удалена из отделения
        assertTrue(branch.getQueues().isEmpty());

        // Отправлено событие удаления очереди
        verify(eventService).sendChangedEvent(eq("config"), eq(false), isNull(), eq(queue), anyMap(), eq("Delete queue"));
    }
    ...
    void adUpdateSegmentRulesThrowsIfServiceGroupMissing() {
        ...
        SegmentationRuleData rule =
                SegmentationRuleData.builder().id("r1").name("rule").serviceGroupId("sg1").queueId("q1").build();
        HashMap<String, SegmentationRuleData> rules = new HashMap<>();
        rules.put("r1", rule);

        // Ожидаем бизнес-ошибку с кодом 404
        HttpStatusException thrown =
                assertThrows(HttpStatusException.class, () -> branch.adUpdateSegmentRules(rules, eventService));
        assertEquals(HttpStatus.NOT_FOUND, thrown.getStatus());

        // Событие бизнес-ошибки опубликовано
        ArgumentCaptor<Event> captor = ArgumentCaptor.forClass(Event.class);
        verify(eventService).send(eq("*"), eq(false), captor.capture());
        assertEquals("BUSINESS_ERROR", captor.getValue().getEventType());
    }
