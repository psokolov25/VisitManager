...
public class VisitService {
  ...
  public Visit deleteOutcomeService(String branchId, String servicePointId, String serviceId) {
    ...
    if (currentBranch.getServicePoints().containsKey(servicePointId)) {
      ...
      if (servicePoint.getVisit() != null) {
        ...
        if (!visit.getCurrentService().getId().equals(serviceId)) {
          throw new BusinessException(
              String.format("Current service ID is not %s@", serviceId), eventService);
        }
  ...
  /**
   * Вызов визита с максимальным временем ожидания из очередей, чьи идентификаторы указаны в @param queueIds
   *
   * @param branchId идентификатор отделения
   * @param servicePointId идентификатор точки обслуживания
   * @param queueIds идентификаторы очередей
  ...
  public Visit visitTransferToUserPool(
      String branchId, String servicePointId, String userId, HashMap<String, String> serviceInfo) {
    Branch currentBranch = branchService.getBranch(branchId);
