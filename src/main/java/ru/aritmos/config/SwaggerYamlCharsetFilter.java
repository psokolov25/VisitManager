...
/**
 * Гарантирует корректное указание кодировки UTF-8 для YAML-спецификаций OpenAPI, отдаваемых Swagger
 * UI. Без явного указания браузер интерпретирует документ в ISO-8859-1, из-за чего вместо русских
 * символов отображаются «крякозябры».
 */
@Filter("/swagger/**")
public final class SwaggerYamlCharsetFilter implements HttpServerFilter {
  ...
  private void enforceUtf8Charset(MutableHttpResponse<?> response) {
    ...
    response
        .getHeaders()
        .set(HttpHeaders.CONTENT_TYPE, yamlContentTypeWithCharset(currentContentType));
  }
