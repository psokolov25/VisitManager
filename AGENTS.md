...
- Предпочтительно Conventional Commits: `feat(scope): …`, `fix: …`, `refactor: …`, `docs: …`, `chore: …`. Сообщения коммитов формулируйте исключительно на русском языке, подробно и детализированно описывайте изменения, придерживайтесь настоящего времени.
