...
- Соблюдайте существующий стиль оформления `README.md`:
  - Текст документа ведётся на русском языке; вверху располагается краткое описание сервиса без заголовка `#`, сразу после него — блок бейджей Shield.io (Java, Micronaut, Build, Tests, Docs, Coverage, Docker, License, Contributing).
  - Содержание оформляется разделом `## 📑 Содержание` с иерархическим списком, повторяющим все основные разделы и подразделы.
  - Заголовки уровней `##` и `###` начинаются с emoji, соответствующих тематике раздела, и совпадают с пунктами содержания.
  - Ссылки на исходный код и ресурсы приводятся в формате `【F:путь†Lначало-Lконец】` сразу после соответствующего предложения или пункта.
  - Команды, HTTP-запросы и примеры кода оформляйте в инлайновом коде или в блоках с указанием языка (например, \`\`\`bash ...\`\`\`, \`\`\`java ...\`\`\`, \`\`\`xml ...\`\`\`).
  - Последовательности действий и сценарии описывайте нумерованными списками с вложенными шагами, сохраняя существующую структуру подпунктов.
  - Раздел «## 🧪 Тестирование» начинается с краткой сводки (команда запуска, покрытие), далее содержит подразделы «### Модульные тесты» и «### Интеграционные тесты» с группировкой по категориям через заголовки `####` и перечислением тестовых классов.
  - Диаграммы подключаются как SVG-изображения из `docs/diagrams/*.svg` с подписями и ссылками на исходники PlantUML.
  - Руководства по ролям, сценарии и документация для автотестеров оформляются как списки с перечислением HTTP-эндпоинтов и кодов ответов в формате, показанном в текущей версии файла.
- При подготовке или обновлении `README.md` явно отражайте, что Keycloak задействован для интеграции и получения данных пользователей, а не для авторизации, и описывайте соответствующие процессы.
